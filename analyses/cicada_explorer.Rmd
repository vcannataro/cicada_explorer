...
cicada_data %>%
  count(obs_date) %>%
  ggplot() + 
  geom_col(aes(y=n,x=obs_date)) + 
  labs(y="Number of observations on iNaturalist", x="Date",
       title="BroodX cicada observations on iNaturalist",
       subtitle="Includes species Magicicada septendecim, Magicicada cassinii, and Magicicada septendecula", caption = "Plot by @VinCannataro") + 
  theme_minimal()

ggsave("output_data/cicada_obs_over_time.png",height = 4,width = 8)
...
ggsave(filename ="output_data/cicada_BroodX_distribution.png",height = 4,width = 8)
```
...
```{r}
save.image(file = "output_data/cicada_data_after_knit.RData")
```
